...
from workers.extraction.layout_engine import layout_extract_pdf_values, layout_rows_from_text_page
...
def save_reports(payload: Dict[str, Any], ctx: RunContext) -> Dict[str, Any]:
    ...
    sql_lines: List[str] = [
        ...
        "-- Review-only SQL template. Direct database save uses pyodbc parameter binding.",
        "-- Do not execute this file as a literal data script; use the generated JSON or the application save command."
    ]
    ...
    for idx, report in enumerate(reports, start=1):
        ...
        sql_lines.append(build_upsert_template_for_review(row))
...
def build_upsert_template_for_review(row: Dict[str, Any]) -> str:
    """Return a review-only parameterized SQL template plus parameter values.

    Direct database writes use pyodbc placeholders in execute_update_balnshet()
    and execute_insert_balnshet(). This function deliberately does not build an
    executable SQL statement containing literal values.
    """
    cols = [c for c in BALNSHET_DB_FIELDS if c != "TransactionNumber"]
    ...
    insert_params = ", ".join("?" for _ in cols)
    update_params = [coerce_db_value(c, row.get(c)) for c in update_cols] + [
        row.get("Symbol"),
        coerce_db_value("FinancialYear", row.get("FinancialYear")),
        coerce_db_value("PeriodEndDate", row.get("PeriodEndDate")),
        row.get("ReportType"),
    ]
    insert_values = [coerce_db_value(c, row.get(c)) for c in cols]
    review = {
        "key": {
            "Symbol": row.get("Symbol"),
            "FinancialYear": coerce_db_value("FinancialYear", row.get("FinancialYear")),
            "PeriodEndDate": coerce_db_value("PeriodEndDate", row.get("PeriodEndDate")),
            "ReportType": row.get("ReportType"),
        },
        "updateParameters": update_params,
        "insertParameters": insert_values,
    }
    return f"""-- Parameterized upsert template for review only
UPDATE [dbo].[BalnShet]
SET {set_sql}
WHERE ISNULL([Symbol], '') = ISNULL(?, '')
  AND ISNULL([FinancialYear], 0) = ISNULL(?, 0)
...
IF @@ROWCOUNT = 0
BEGIN
    INSERT INTO [dbo].[BalnShet] ({insert_cols}) VALUES ({insert_params});
END
...
# Backwards-compatible name used by older callers. It now returns a parameterized
# review template rather than executable SQL with literal values.
def build_insert_sql(row: Dict[str, Any]) -> str:
    return build_upsert_template_for_review(row)
        ...
        try:
            ...
            try:
                ...
                if looks_like_text_short_exception(legacy_exc):
                    ...
                    try:
                        values, evidence, warnings = layout_extract_pdf_values(pdf_path, report, year, symbol, scanned_ocr_callback=ocr_extract_scanned_pdf_ocr_values)
            ...
            try:
                ...
                if should_try_layout_fallback(values, _mapping_text, symbol, report, base_score):
                    layout_values, layout_evidence, layout_warnings = layout_extract_pdf_values(pdf_path, report, year, symbol, scanned_ocr_callback=ocr_extract_scanned_pdf_ocr_values)
